							...
							case 2:
								if(tipoCuenta.equals("CREDITO")) {
									...
									response.setCodigo(200);
									 ...
									 response.setCodigo(409);
									 response.setMensaje("Se busca un producto que no es credito");
									 log.info("..........Se desea obtener detalles de credito pero se inserto uno que no lo es...........");
								 }
							   break;	
						   }				
					    }else {
					    	response.setCodigo(409);
					    	response.setMensaje("Producto no es un credito");
					    	log.info(".......Producto no es un credito.......");
					    }
				    }else {
					  response.setCodigo(409);
					  response.setMensaje("Cuenta inactiva");
        	 ...
        	 if(opa.getIdorigenp() > 0 && opa.getIdproducto() > 0 && opa.getIdauxiliar() > 0) {
            	 ...
            	 if(auxiliar != null) {
            		 if(auxiliar.getEstatus() == 2) {
                		...
                		response.setCodigo(409);
                		response.setMensaje("Cuenta esta inactiva");
