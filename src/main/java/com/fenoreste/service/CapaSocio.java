	...
	public ResponseSocio buscarSocioPorOgs(String ogs) {
		...
		try {
			...
			if(persona != null) {
					...
					if(aux_parte_social != null && aux_parte_social.getEstatus() == 2) {
			...
			if(persona.getPk() != null) {			
				...
				for(int i=0;i<_Folios.size();i++) {
				  ...
				  if(p.getIdproducto() != 100) {
					  ...
					  if(p.getNombre().toUpperCase().indexOf("AHORRO") != -1){						  
						  ...
						  res.setFechaApertura(util.convertFechaDate(a.getFechaactivacion())+"Z");
					  ...
					  }else if(p.getNombre().toUpperCase().contains(tipoCuenta.toUpperCase()) || p.getNombre().toUpperCase().contains("DEPOSI")) {
						  ...
						  res.setSaldos(saldos);
						  res.setDatoInversion(inversion_res);
						  listaCuentas.add(res);
						  ...
						  res.setTipoCuenta("PERSONAL");						  
						  res_saldo = new ResponseSaldo();
						  res.setTipoCuenta("CREDITO");
						  res.setSubtipoCuenta("CREDITO");
						  res.setNivelOperacion("DEPOSITOS");				  
						  ...
						  res_credito.setFechaOrigen(util.convertFechaDate(a.getFechaactivacion())+"Z");
						  ...
						  res.setDatoCredito(res_credito);
						  res.setSaldos(saldos);
						  listaCuentas.add(res);
					   }
				  	}
